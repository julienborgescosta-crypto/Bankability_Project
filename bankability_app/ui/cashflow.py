...
def render(inputs: ProjectInputs, result: ProjectResults) -> None:
    ...
    fig = go.Figure()
    fig.add_bar(x=df["Year"], y=df["Revenues"], name="Revenues", marker_color="#2f6f4f")
    fig.add_bar(x=df["Year"], y=df["OPEX"], name="OPEX", marker_color="#b23b3b")
    fig.add_bar(x=df["Year"], y=df["TURPE"], name="TURPE", marker_color="#c98a3b")
    fig.add_bar(x=df["Year"], y=df["CAPEX"], name="CAPEX", marker_color="#555555")
    fig.add_trace(
        go.Scatter(
            x=df["Year"],
            y=df["Net Cashflow"],
    ...
    fig.update_layout(
        barmode="relative", title="Annual cashflow (k€, nominal)", legend={"orientation": "h"}
    )
    ...
    fig_dscr.add_hline(
        y=1.30, line_dash="dash", line_color="#c98a3b", annotation_text="Usual bank threshold 1.30x"
    )
    fig_dscr.update_layout(title="Annual DSCR", yaxis_title="DSCR (x)")
